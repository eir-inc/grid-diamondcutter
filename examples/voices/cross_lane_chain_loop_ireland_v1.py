...
"""
cross_lane_chain_loop_ireland_v1.py — §3.1 polyphony voice.
...
as INPUT to predict an inflection-year window for Ireland RE-share trajectory,
...
Ireland is a NON-LEADER cohort country with a documented FIT policy year:
  - 2006 White Paper Delivering a Sustainable Energy Future
  - 2009 NREAP National Renewable Energy Action Plan — principal RES anchor
  - 2015 EU 2030 framework adoption

Using miles's calibrated prior (8.83y ± 1.47y MAE), predicted inflection-year
window for Ireland:
  policy_year: 2009 (Ireland principal NREAP). Ireland sibling test to cajal #48 Greece — both Eurozone-crisis 2010-2013 cohort.
  prior_mean: 8.83y
...
milestones for Ireland RE-share. v2-ratchet to real Eurostat / ENTSO-E
...
VOICE_NAME = "cross_lane_chain_loop_ireland_v1"
...
IRELAND_POLICY_ANCHOR_YEAR = 2009  # Ireland NREAP (National Renewable Energy Action Plan), principal RE policy anchor

_pred_lo = IRELAND_POLICY_ANCHOR_YEAR + MILES_LEAD_TIME_PRIOR["mean_lead_time_years"] - MILES_LEAD_TIME_PRIOR["loo_mae_years"]
_pred_hi = IRELAND_POLICY_ANCHOR_YEAR + MILES_LEAD_TIME_PRIOR["mean_lead_time_years"] + MILES_LEAD_TIME_PRIOR["loo_mae_years"]

PREDICTION = {
    "kind": "polyphony_within_substrate",
    "substrate": "ireland_renewable_share_trajectory_2010_2023_synthetic_v1_for_cross_lane_chain_test",
    "named_residual": "ireland_re_share_inflection_year_falls_within_miles_prior_calibrated_window",
    "policy_anchor_year": IRELAND_POLICY_ANCHOR_YEAR,
    ...
    "rationale": (
        ...
        "calibrated prior to Ireland (non-leader cohort, documented FIT policy "
...
KILL_CONDITION = {
    "metric": "ireland_inflection_year_validity_under_miles_prior",
    "rule": (
        "fail if recovered_breakpoint_year outside predicted_inflection_window "
        "(prior does not generalize to Ireland); "
...
RUN_PROTOCOL = {
    "entry_point": "python examples/voices/cross_lane_chain_loop_ireland_v1.py",
    "source_file": "examples/voices/cross_lane_chain_loop_ireland_v1.py",
    "input_parameters": {
        "calibration_source": "synthetic_v1_from_publicly_cited_IEA_Eurostat_milestones",
        # Ireland RE share of electricity generation (% of total, synthetic-v1 from
        # publicly-cited Eurostat + IEA — GR was ~17% RE in 2010, reached ~31%
        # by 2020 post-2014 plateau exit, ~46% by 2023)
        "ireland_re_share_milestones": {
            "2010": 0.14, "2012": 0.19, "2014": 0.24, "2016": 0.26, "2018": 0.33,
            "2020": 0.42, "2022": 0.38, "2023": 0.41,
        },
        "policy_anchor_year": IRELAND_POLICY_ANCHOR_YEAR,
...
def run_voice() -> dict:
    ...
    milestones = RUN_PROTOCOL["input_parameters"]["ireland_re_share_milestones"]
    ...
    return {
        ...
        "ireland_re_share": [float(s) for s in shares],
...
def compute_verdict(run_output: dict) -> dict:
    ...
    if bp is None or bp < bp_lo or bp > bp_hi:
        fails.append(
            ...
            f"window [{bp_lo}, {bp_hi}] (chain does not close at Ireland)"
        )
    ...
    if not fails:
        ...
        rationale = (
            ...
            f"(#30 → #33 → #40) generalizes to Ireland at cross-lane consumer "
...
def main():
    ...
    print(f"Ireland policy anchor: {IRELAND_POLICY_ANCHOR_YEAR}")
    print(f"predicted inflection window: {PREDICTION['predicted_inflection_window']}")
    print(f"kill rule: {KILL_CONDITION['rule']}")
    print()
    print("running Ireland inflection-year recovery under miles prior...")
    out = run_voice()
    print(f"  ireland RE share milestones: {RUN_PROTOCOL['input_parameters']['ireland_re_share_milestones']}")
